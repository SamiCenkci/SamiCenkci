...
|---------|-------------|------|
| **[Wisp](https://github.com/SamiCenkci/Shopping-Website)** · [Live](https://wispapp.net) | Full-stack secondhand marketplace for the Norwegian market, inspired by Finn.no. Built end-to-end: JWT auth with email verification, a three-level category tree with per-branch attributes (JSONB), typo-tolerant search (PostgreSQL `pg_trgm`), buyer–seller chat with image and file attachments, mutual reviews between buyer and seller, favorites with price-drop emails, saved searches with email alerts, admin moderation, Norwegian/English language switching, and light/dark mode. Kartverket address autocomplete with map previews, images via S3 presigned uploads. Deployed across Vercel, Render and Neon behind a custom domain. | Next.js, TypeScript, Go (Gin), PostgreSQL (Neon), SQLC, AWS S3, Resend, Vercel, Render |
